...
export default function CountrySelect() {
  ...
  const handleChange = (event,value) => {
    ...
    const updateStore = (validationStatus) =>{
        const payload = {fieldName:'country', fieldValue:{value: selectedCountry,validationStatus: validationStatus}}  
    ...
    const handleValidate = event => {
        ...
        if(event.target.value === ""){
            setValidation({status:true, msg:'Country is required'});
            updateStore(true)
        }else{
    ...
    resetCountry = ()=> {
      setCountry("");
      const payload = {fieldName:'country', fieldValue:{value: "",validationStatus: true}}  
      dispatch(updateDrawerMailFormData(payload))
    }
